...
run_task() {
    ...
    python train.py --seed 42 --train_diag_path "${DATA_PATH}/task${task_num}.ddata.wst.txt" >> "$log_file" 2>&1
    
    # 평가 실행
    for i in $(seq 1 $task_num); do
        echo "Evaluating on Task $i" | tee -a "$log_file"
        python eval.py --seed 42 --val_diag_path "${DATA_PATH}/cl_eval_task${i}.wst.dev" >> "$log_file" 2>&1
    done
